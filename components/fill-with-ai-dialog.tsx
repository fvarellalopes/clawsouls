    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-surface border-border max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-fg">{t("description")}</DialogDescription>
        </DialogHeader>
        <div className="space-y-6">
          <div className="space-y-2">
            <Label className="text-fg text-sm">{t("bulletPoints")}</Label>
            <Textarea value={bulletsText} onChange={(e) => setBulletsText(e.target.value)} placeholder={t("placeholder")} rows={5} className="bg-surface-alt/80 border-border focus:border-primary rounded-xl resize-none text-fg placeholder:text-muted-fg" />
            <p className="text-xs text-muted-fg">{t("hint")}</p>
          </div>
          {!isGenerated && (
            <Button onClick={handleGenerate} disabled={!bulletsText.trim()} className="w-full bg-primary text-primary-fg border-0 h-12 text-base font-display tracking-wider">
            ...
            <div className="space-y-5 animate-fade-up">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => handleGenerate()} className="text-muted-fg hover:text-fg h-7 px-2">
                    <RefreshCw className="h-3.5 w-3.5 mr-1" />
                    {t("regenerate")}
                  </Button>
                </div>
                <div className="p-4 rounded-xl bg-surface-alt/80 border border-border text-fg text-sm leading-relaxed whitespace-pre-wrap">{generatedVibe}</div>
              </div>
                ...
                <div className="p-4 rounded-xl bg-surface-alt/80 border border-border space-y-4">
                    ...
                    return (
                      <div key={attr} className="space-y-1.5">
                        <div className="flex justify-between items-center">
                          <span className="text-xs text-muted-fg capitalize">{attrLabels[attr] || attr}</span>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] text-muted-fg font-mono">{current}</span>
                            <span className="text-muted-fg">→</span>
                            <span className="text-xs text-accent font-mono bg-accent/10 px-1.5 py-0.5 rounded">{suggested}</span>
                          </div>
                        </div>
                        <div className="relative h-2">
                          <div className="absolute inset-0 rounded-full bg-border" />
                          <div className="absolute top-0 left-0 h-full rounded-full bg-primary/20 transition-all" style={{ width: `${current}%` }} />
                          <div className="absolute top-0 left-0 h-full rounded-full bg-primary/40 transition-all" style={{ width: `${suggested}%` }} />
                        </div>
                      </div>
                    );
                  })}
                  {suggestedAttrs.communicationMode && (
                    <div className="flex justify-between items-center pt-2 border-t border-border">
                      ...
                      <span className="text-xs text-accent font-mono bg-accent/10 px-1.5 py-0.5 rounded">{suggestedAttrs.communicationMode}</span>
                    </div>
                  )}
                  {suggestedAttrs.vibeStyle && (
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-muted-fg">Vibe Style</span>
                      <span className="text-xs text-accent font-mono bg-accent/10 px-1.5 py-0.5 rounded">{suggestedAttrs.vibeStyle}</span>
                    </div>
                  )}
                  {suggestedAttrs.knowledgeDomains && suggestedAttrs.knowledgeDomains.length > 0 && (
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs text-accent font-mono bg-accent/10 px-1.5 py-0.5 rounded">{suggestedAttrs.knowledgeDomains.join(", ")}</span>
                    </div>
                  )}
                </div>
              </div>
              <div className="flex gap-3">
                <Button onClick={() => handleGenerate()} variant="outline" className="flex-1 border-border text-fg">
                  <RefreshCw className="mr-2 h-4 w-4" />
                  {t("regenerate")}
                </Button>
                <Button onClick={handleApply} className="flex-1 bg-primary text-primary-fg border-0">
